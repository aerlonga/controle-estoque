...
async function createUsers() {
    ...
    for (let i = 0; i < USUARIOS; i++) {
        // Garantir usuario_rede único adicionando o índice
        const baseUsername = faker.internet.username().toLowerCase();
        const uniqueUsername = `${baseUsername}_${i}`;

        const user = await prisma.usuario.create({
            data: {
                nome: faker.person.fullName(),
                usuario_rede: uniqueUsername,
                senha_hash: hashedPassword,
...
async function createEquipments(users) {
    ...
    for (let i = 0; i < EQUIPAMENTOS; i++) {
        ...
        // Numero de serie: aceita letras e números (alphanumeric)
        const serialNumber = `${tipoEquipamento.substring(0, 3).toUpperCase()}${faker.string.alphanumeric(8).toUpperCase()}`;

        // Patrimonio: APENAS NÚMEROS (sem prefixo "PAT-")
        const patrimonioNumber = Math.random() > 0.2 ? String(100000 + i).padStart(8, '0') : null;
...
async function main() {
    console.log('==========================================');
    console.log('🚀 INICIANDO SEED FAKE DO BANCO DE DADOS');
    console.log('==========================================');
    ...
    try {
        ...
        console.log('==========================================');
        console.log('✅ SEED FAKE CONCLUÍDO COM SUCESSO!');
        console.log('==========================================');
